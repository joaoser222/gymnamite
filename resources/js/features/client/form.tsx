...
export default function ClientFormModal({ opened, mode, editValues, onClose, onSubmit, onSuccess }: ClientFormModalProps) {
  ...
  useEffect(() => {
    if (!opened) return;
    if (editValues) form.setValues({ ...initialValues, ...editValues });
    else form.reset();
  }, [opened]);
  ...
  return (
    ...
    >
      <Grid>
        ...
        <Grid.Col span={{ base: 12, sm: 6 }}>
          <PhoneInput label="Telefone" {...form.getInputProps('phone')} />
        </Grid.Col>
        <Grid.Col span={{ base: 12, sm: 6 }}>
          <PhoneInput label="Telefone" {...form.getInputProps('phone')} />
        </Grid.Col>
        <Grid.Col span={{ base: 12, sm: 6 }}>
          <DateInput label="Data de Nasc." placeholder="DD/MM/YYYY" valueFormat="DD/MM/YYYY" {...form.getInputProps('birth_date')} />
        </Grid.Col>
