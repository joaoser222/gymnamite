<template>
  ...
  >
    <template #append-inner>
      <v-menu location="end" :close-on-content-click="false" v-model="datePickerMenu">
        <template v-slot:activator="{ props }">
          <v-btn icon="ti ti-calendar" v-bind="props" size="sm"  tabindex="-1"></v-btn>
        </template>
        <v-date-picker elevation="24" color="primary" @update:modelValue="datePickerInput"></v-date-picker>
      </v-menu>
    </template>
  </v-text-field>
...
function datePickerInput(date: string): void {
